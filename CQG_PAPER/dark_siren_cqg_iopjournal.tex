...
To probe photo-$z$ failure more directly we anchor a fraction of the host-weight proxy to spectroscopic redshifts by overriding catalogue redshifts where spec-$z$ matches exist. Under strict shifted-sky false-match controls (median shifted/true $<0.1$; max $<0.3$), the clean operating point at the maximal common depth ($r=10^{\prime\prime}$, $K=20000$) anchors $\simeq 4$--$8\%$ of the host-weight proxy across the top three events (median $\simeq 6\%$). The score remains stable or slightly higher ($\Delta\LPD_{\rm tot}\simeq 3.63 \rightarrow 3.64$) under unchanged selection normalisation. Across the tested clean $(r,K)$ grid, $\Delta\LPD_{\rm tot}$ is non-decreasing with spec-$z$ weight coverage.
...
Analysis artifacts and summary tables are provided with this repository and mirrored on the associated project archive (Zenodo DOI: 10.5281/zenodo.18640507).
