...
\textbf{Spectroscopic redshift override.} To stress-test sensitivity to global photo-$z$ bias in the dominant distance--redshift channel, we construct a ranked list of host candidates for each high-leverage event under the same host-weight proxy used in spectral-only re-scoring, crossmatch their coordinates against SDSS DR16 spectroscopic objects via the CDS XMatch service, and override the catalogue redshift with $z_{\rm spec}$ where available. We report both the fraction of total host-weight proxy covered by spectroscopic matches and the resulting $\Delta\LPD$ under the unchanged selection normalisation, together with shifted-sky false-match controls to identify coincidence-dominated match radii.
