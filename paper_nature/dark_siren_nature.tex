...
To narrow the remaining dismissal pathways, we extended the stress-testing programme with adversarial and split-coherence checks using cached-term re-scoring (Fig.~\ref{fig:robustness_summary}). A flexible nuisance deformation of the injection-calibrated selection function, implemented as bounded multiplicative splines in effective SNR and redshift with a mild mass tilt, does not erase the preference: across tested bounds up to $\pm60\%$ in this nuisance family, the best-achieved score remains $\Delta\LPD_{\rm tot}\simeq+3.67$ and never falls below 1 or 0. In the dominant distance--redshift channel (spectral-only re-scoring, which retains most of the baseline preference), global photometric-redshift biases modelled as $\Delta z=b_0+b_1 z$ can reduce the preference but do not remove it on the tested grid: even an extreme choice ($b_0=0.03$, $b_1=0.2$) yields $\Delta\LPD\simeq+1.98$, while completeness-weight tilts at the $\pm20\%$ level leave $\Delta\LPD\gtrsim+3.61$. To probe photo-$z$ failure more directly, we crossmatch the top-weight host candidates for the high-leverage events against public spectroscopic redshift resources including DESI DR1, and override catalogue redshifts where matches exist; at $K=20000$ and $r\le30^{\prime\prime}$ this anchors up to $\simeq9\%$ (GW200308\_173609), $\simeq5\%$ (GW200220\_061928), and $\simeq10\%$ (GW200219\_094415) of total host-weight proxy to spectroscopic redshifts, with median matched host-weight fraction $\simeq5\%$ in the best-scored condition. The score remains stable or slightly higher ($\Delta\LPD_{\rm tot}\simeq3.63\rightarrow3.65$; best 3.65 at $r=30^{\prime\prime}$) under unchanged selection normalisation. Across the tested $(r,K)$ grid, $\Delta\LPD_{\rm tot}$ is non-decreasing with spec-$z$ weight coverage (pooled slope $b\simeq0.40$, $p\simeq10^{-10}$, i.e.\ $\approx4\times10^{-3}$ in $\Delta\LPD$ per +1\% coverage). A shifted-sky false-match control confirms that coincidence rates rise rapidly with match radius once DESI is included: $60^{\prime\prime}$ matches are coincidence-dominated (median shifted/true $\simeq0.72$) and are excluded by the gate, while $3^{\prime\prime}$ remains clean (median shifted/true $\simeq2\times10^{-3}$) and $10^{\prime\prime}$ remains below $\simeq0.05$; we treat $r\le10^{\prime\prime}$ as the conservative regime for this audit. Swapping public GWTC-3 parameter-estimation analysis groups for the two highest-leverage events (GW200308\_173609 and GW200220\_061928) leaves the global score stable at $\Delta\LPD\simeq+3.63$ across IMRPhenomXPHM, SEOBNRv4PHM and mixed analyses. A binned-redshift scramble null in the same spectral-channel approximation weakens the preference but does not collapse it (null mean $\Delta\LPD_{\rm tot}\approx+3.38\pm0.15$ versus observed $+3.65$, empirical $p\approx0.05$), indicating some dependence on coherent catalogue-redshift structure while leaving residual degeneracy with modelling assumptions. Finally, strictly non-circular splits show that the preference concentrates at larger inferred distances: in three equal-count bins of the GW luminosity-distance posterior median, $\Delta\LPD\approx-0.52$, $+0.57$ and $+3.55$ from low to high distance, consistent with a cumulative propagation effect.
...
\textbf{Spectroscopic redshift override.} To stress-test sensitivity to global photo-$z$ bias in the dominant distance--redshift channel, we construct a ranked list of host candidates for each high-leverage event under the same host-weight proxy used in spectral-only re-scoring, crossmatch their coordinates against SDSS DR16 spectroscopy via the CDS XMatch service and against additional public spec-$z$ catalogues (2MRS, 6dFGS, 2dFGRS and GAMA DR3), and override the catalogue redshift with $z_{\rm spec}$ where available. We report both the fraction of total host-weight proxy covered by spectroscopic matches and the resulting $\Delta\LPD$ under the unchanged selection normalisation, together with shifted-sky false-match controls to identify coincidence-dominated match radii. We additionally re-score under strict match-quality tiers (e.g.\ SDSS clean spectra with high-quality galaxy classifications) to quantify sensitivity to spurious matches.

\textbf{Code and data availability.} Analysis artifacts and summary tables are provided with this repository and mirrored on the associated project archive (Zenodo DOI: 10.5281/zenodo.18604204).
